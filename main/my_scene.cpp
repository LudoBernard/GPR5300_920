...
namespace gpr5300
{
	...
	void ModelScene::Begin()
	{
		...
		glFrontFace(GL_CCW);
			...
			model_.view = translate(model_.view, glm::vec3(0.0f, 0.0f, -2.f));
	...
	void ModelScene::Update(float dt)
	{
		...
		glm::mat4 model = scale(model_.model, glm::vec3(0.2f, 0.2f, 0.2f));	// it's a bit too big for our scene, so scale it down
